...
void io_handler(void *io_area, size_t io_size) {
    putchar((int) *((char *) io_area));
}

using namespace Ceramium;

const HMem_Area_Specifier mem = {(void *) &vm_guest_code, sizeof(vm_guest_code)};
...
int main(int argc, const char **argv) {
    ...
    // Reset the Core: Makes sure we leave the vector segment and are in real address space
    _ceratest.Reset_Core(_core1);
    // Run
